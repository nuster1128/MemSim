...
class RecentMemAgent(BaseAgent):
    ...
    def observe_without_action(self, obs):
        obs = '[%d] %s' % (self.memory_index, obs)
        self.memory_list = self.memory_list[-(self.config['size']-1):] + [obs]
        self.memory_index += 1
    ...
    def response_retri(self, question, topk = 5):
        ...
        try:
            ...
            return np.random.choice(range(len(self.memory_list)), size=topk, replace=True).tolist()
